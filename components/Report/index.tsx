...
export function Report({ report }) {
  ...
  const now = new Date();
  const date = reportGeneratedDate.toLocaleDateString("en-HK", {
    month: "short",
    day: "numeric",
    ...(reportGeneratedDate.getFullYear() !== now.getFullYear() && { year: "numeric" }),
  });
  return (
    <li>
      ...
      <div className="mx-8 flex justify-between">
        ...
        <span className="inline-flex rounded-md shadow-sm">
          <button
            onClick={() => dispatch({ type: "viewReport", payload: report.id })}
            type="button"
          ...
          <div className="flex items-center justify-between space-x-3">
            <h2 className="text-lg leading-7 font-medium text-white">
              {!loading && data.report.is_final && (
                <FontAwesomeIcon className={`mr-2 text-green-400`} icon={["fad", "badge-check"]} size="lg" />
              )}
            ...
            <div className="h-7 flex items-center">
              <button
                ...
                className="text-indigo-200 hover:text-white focus:outline-none transition ease-in-out duration-150"
              >
              ...
              <div className="space-y-1">
                <div className="relative bg-gray-700 rounded-lg shadow-md px-4 py-3">
                  ...
                  <span className="text-xs text-gray-100">
                    on
                    <pre className="ml-1 font-mono inline-block">
                      submissions/{!loading && data.report.submission.upload_name}
                    </pre>
                  </span>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("diffWithSkeleton") && (
                        <DiffWithSkeletonStageView reports={data.report.sanitizedReports.diffWithSkeleton} />
                      )}
                  </div>
                  <div className="space-y-1">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("fileStructureValidation") && (
                        <FileStructureValidationStageReportView
                          reports={data.report.sanitizedReports.fileStructureValidation}
                        />
                      )}
                  </div>
                  <div className="space-y-1">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("compile") && (
                        <CompileStageReportView reports={data.report.sanitizedReports.compile} />
                      )}
                  </div>
                  <div className="space-y-1">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("make") && (
                        <MakeStageReportView reports={data.report.sanitizedReports.make} />
                      )}
                  </div>
                  <div className="space-y-2">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("gTest") && (
                        <GTestStageReportView reports={data.report.sanitizedReports.gTest} />
                      )}
                  </div>
                  <div className="space-y-2">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("jUnit") && (
                        <JUnitStageReportView reports={data.report.sanitizedReports.jUnit} />
                      )}
                  </div>
                  <div className="space-y-2">
                    {data.report.sanitizedReports !== null &&
                  ...
                  <div className="space-y-2">
                    {data.report.sanitizedReports !== null &&
                      Object.keys(data.report.sanitizedReports).includes("valgrind") && (
                        <ValgrindStageView reports={data.report.sanitizedReports.valgrind} />
                      )}
                  </div>
                  <div className="space-y-2">
      ...
      <div className="flex-shrink-0 px-4 py-4 space-x-4 flex justify-end">
        <span className="inline-flex rounded-md shadow-sm">
          <button
            onClick={() => dispatch({ type: "closeSlideOver" })}
            type="submit"
            className="inline-flex justify-center py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-cse-600 hover:bg-cse-500 focus:outline-none focus:border-cse-700 focus:shadow-outline-indigo active:bg-cse-700 transition duration-150 ease-in-out"
          >
            Done
          </button>
        </span>

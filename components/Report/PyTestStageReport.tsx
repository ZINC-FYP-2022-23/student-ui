...
function PyTestTestSuiteRowItem({ testsuite }) {
  ...
  const visibleTestCases = testsuite.testcases.filter(
    (testcase) =>
      !(
        testcase.failures.length !== 0 &&
        testcase.failures.filter((failure) => failure.category === "SKIPPED").length > 0
      ),
  );
  return (
    <li className="my-2">
      ...
      <header className="bg-gray-100 w-full px-4 py-3 rounded-tl-lg rounded-tr-lg border-b flex justify-between items-center">
        <div>
          ...
          <h6 className="text-xs text-gray-600">
            Score:
            {reports[viewingIndex]["score"] === null ? (
              0
            ) : (
              <span className="ml-1 font-medium">
                {reports[viewingIndex]["score"]["score"].toFixed(1)}/{reports[viewingIndex]["score"]["total"]}
              </span>
            )}
          </h6>
        </div>
        <span className="bg-blue-400 w-10 h-10 rounded-full flex justify-center items-center shadow">
        ...
        {reports[viewingIndex]["report"] === null ? (
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm leading-5 text-red-700">
                  <ul className="list-disc pl-5">
                    <li>The report for this stage is unavailable due to:</li>
                    <li>Automatic grader ran into a problem while running PyTest</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <ul>
            {reports[viewingIndex]["report"]["testsuites"].map((testsuite) => (
              <PyTestTestSuiteRowItem testsuite={testsuite} key={testsuite.displayName} />
            ))}
          </ul>
        )}
      </div>

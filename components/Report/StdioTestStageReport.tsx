...
function StdioTestStatus({ allPassed }) {
  return (
    ...
    >
      <FontAwesomeIcon
        className={`${allPassed ? "text-green-500" : "text-red-500"}`}
        icon={["far", allPassed ? "check" : "times"]}
      />
    </span>
  );
}
        ...
        <ul>
          {reports.map((testCase, i) => (
            <li className="flex items-center justify-between my-2" key={i}>
              <div>
                <FontAwesomeIcon
                  className={`mr-2 ${testCase.isCorrect ? "text-green-500" : "text-red-500"}`}
                  icon={["far", testCase.isCorrect ? "check" : "times"]}
                />
                <span className="text-sm">Test #{testCase.id}</span>
              </div>
              <span className="inline-flex rounded-md shadow-sm">
                <button
                  onClick={() => dispatch({ type: "viewStdioComparison", payload: testCase })}
            ...
            <div className="mt-3 text-center sm:mt-0 sm:mx-4 sm:text-left flex-1">
              <div className="flex justify-between">
                <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
                  <FontAwesomeIcon
                    className={`${stdioTestCase.isSuccess ? "text-green-500" : "text-red-500"} mr-2`}
                    icon={["far", stdioTestCase.isSuccess ? "check" : "times"]}
                  />
                  ...
                  <div className="hidden sm:block">
                    <nav className="flex">
                      <button
                        ...
                        className={`ml-4 px-3 py-2 flex items-center font-medium text-sm leading-5 rounded-md ${
                          viewingMode === "error"
              ...
              <div className="mt-4 rounded-lg bg-gray-700 shadow-inner border p-3 text-white font-mono h-56 overflow-y-auto w-full">
                ...
                {stdioTestCase.stderr.length === 0 && <span>No Output</span>}
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 text-center sm:mt-0 sm:text-left flex-1 w-full">
              ...
              <div className="hidden sm:block">
                <nav className="flex">
                  <button
                    ...
                    className={`px-3 py-2 font-medium text-sm leading-5 rounded-md ${
                      viewingMode === "stdout"
                        ? "text-gray-700 bg-gray-100 focus:bg-gray-200"
                        : " text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:bg-gray-100"
                    } focus:outline-none`}
                  ...
                  <button
                    ...
                    className={`ml-4 px-3 py-2 font-medium text-sm leading-5 rounded-md ${
                      viewingMode === "split"
                        ? "text-gray-700 bg-gray-100 focus:bg-gray-200"
                        : " text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:bg-gray-100"
                    } focus:outline-none`}
            ...
            {viewingMode === "stdout" && (
              <div className="flex flex-col">
                ...
                {!stdioTestCase.isSuccess &&
                  stdioTestCase.hasOwnProperty("exeExitCode") &&
                  stdioTestCase.exeExitCode !== 0 && (
                    <div className="mt-2">
                      <span className="inline-flex items-center px-2.5 py-1.5 rounded-md text-sm font-medium leading-5 bg-gray-100 text-gray-800">
                        ...
                        <span className="ml-1 font-mono">
                          ...
                          <a href="https://man7.org/linux/man-pages/man7/signal.7.html" className="ml-1">
                            (
                            {stdioTestCase.exitCode > 128
                              ? signal[stdioTestCase.exitCode - 128]
                              : signal[stdioTestCase.exeExitCode]}
                            )
            ...
            {viewingMode === "split" && (
              <div className="mt-4">
                {stdioTestCase.visibility === "ALWAYS_VISIBLE" ? (
                  <div className="flex flex-col">
                    <div className="flex justify-between">
                      <span className="flex justify-center items-center font-medium text-xs tracking-wider uppercase bg-gray-100 py-1.5 px-3 text-gray-500 rounded-lg">
                        Expected output
                      </span>
                      <span className="flex justify-center items-center font-medium text-xs tracking-wider uppercase bg-gray-100 py-1.5 px-3 text-gray-500 rounded-lg">
                        Program output

      ...
      <header className="bg-gray-100 w-full px-4 py-3 rounded-tl-lg rounded-tr-lg border-b flex justify-between items-center">
        <div>
          <h3 className="text-sm font-medium leading-5">
            ...
            <span
              aria-label={
                reports[viewingIndex].isMatched
                  ? "Submission matches with specification"
                  : "Submission does not match with specification"
              }
              data-flow="up"
              className="ml-2"
            >
              <FontAwesomeIcon
                className={`text-${reports[viewingIndex].isMatched ? "green" : "red"}-500`}
                icon={["far", reports[viewingIndex].isMatched ? "check" : "times"]}
              />
            </span>
          </h3>
          ...
          <div>
            {reports[viewingIndex].extraPaths.length > 0 && (
              <>
                ...
                <ul>
                  {reports[viewingIndex].extraPaths.map((path) => (
                    <li key={path}>{path}</li>
                  ))}
                </ul>
              </>
          ...
          <div>
            {reports[viewingIndex].missingPaths.length > 0 && (
              <>
                ...
                <ul>
                  {reports[viewingIndex].missingPaths.map((path) => (
                    <li key={path}>{path}</li>
                  ))}
                </ul>
              </>

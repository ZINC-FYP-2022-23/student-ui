...
function ValgrindResultStatus({ allPasses, timedOut }) {
  if (timedOut) {
    return (
      <span aria-label="Compilation has timed out" data-flow="right" className="ml-2">
  ...
  return (
    ...
    >
      <FontAwesomeIcon
        className={`${allPasses ? "text-green-500" : "text-red-500"}`}
        icon={["far", allPasses ? "check" : "times"]}
      />
    </span>
  );
}
  ...
  return (
    <div className="bg-white shadow rounded-lg border py-3">
      <header className="w-full px-4 flex justify-between items-center">
        ...
        <span className="bg-green-400 w-10 h-10 rounded-full flex justify-center items-center shadow">
        ...
        <ul>
          {reports.map((testCase, i) => (
            <li className="flex items-center justify-between my-2" key={i}>
              <div>
                <FontAwesomeIcon
                  className={`mr-2 ${testCase.isSuccess ? "text-green-500" : "text-red-500"}`}
                  icon={["far", testCase.isSuccess ? "check" : "times"]}
                />
                <span className="text-sm">Test #{testCase.id}</span>
              </div>
              <span className="inline-flex rounded-md shadow-sm">
                <button
                  onClick={() => dispatch({ type: "viewValgrindReport", payload: testCase })}
...
export function ValgrindDetailView() {
  ...
  if (valgrindTestCase.visibility === "ALWAYS_HIDDEN") {
    return (
      <>
          ...
          <div className="sm:flex sm:items-start">
            <div className="mt-3 text-center sm:mt-0 sm:text-left flex-1 w-full">
                ...
                <div>
                  <nav className="flex">
                    <button
                      ...
                      className={`ml-4 flex px-3 py-2 tracking-wider font-medium text-sm leading-5 rounded-md ${
                        viewingMode === "stderr"
                          ? "text-gray-700 bg-gray-100 focus:bg-gray-200"
                          : " text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:bg-gray-100"
                      } focus:outline-none`}
                    >
              ...
              <div className="mt-4 rounded-lg bg-gray-700 shadow-inner border p-3 text-white font-mono h-56 overflow-y-auto w-full">
                {valgrindTestCase.stderr.length > 0 && valgrindTestCase.stderr.map((line, i) => <p key={i}>{line}</p>)}
                {valgrindTestCase.stderr.length === 0 && <span>No Output</span>}
              </div>
            </div>
          </div>
      ...
      <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div className="sm:flex sm:items-start">
          <div className="mt-3 text-center sm:mt-0 sm:mx-4 sm:text-left flex-1">
              ...
              <div>
                <nav className="flex">
                  <button
                    ...
                    className={`px-3 py-2 tracking-wider font-medium text-sm leading-5 rounded-md ${
                      viewingMode === "stdout"
                        ? "text-gray-700 bg-gray-100 focus:bg-gray-200"
                        : " text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:bg-gray-100"
                    } focus:outline-none`}
                  ...
                  <button
                    ...
                    className={`ml-4 px-3 py-2 tracking-wider font-medium text-sm leading-5 rounded-md ${
                      viewingMode === "errors"
                        ? "text-gray-700 bg-gray-100 focus:bg-gray-200"
                        : " text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:bg-gray-100"
                    } focus:outline-none`}
            ...
            {viewingMode === "stdout" && (
              <div className="mt-4 rounded-lg bg-gray-700 shadow-inner border p-3 text-white font-mono h-56 overflow-y-auto w-full">
                {valgrindTestCase.stdout.length > 0 && valgrindTestCase.stdout.map((line, i) => <p key={i}>{line}</p>)}
                {valgrindTestCase.stdout.length === 0 && <span>No Output</span>}
              </div>
            )}
            {viewingMode === "stderr" && (
              <div className="mt-4 rounded-lg bg-gray-700 shadow-inner border p-3 text-white font-mono h-56 overflow-y-auto w-full">
            ...
            {viewingMode === "errors" && <ValgrindErrorViewer errors={valgrindTestCase.errors} />}
          </div>
        </div>
        <TimeoutBanner stageName="Valgrind Check" hidden={!valgrindTestCase.hasTimedOut} />
      </div>
                ...
                <thead>
                  <tr>
                    <th className="px-6 py-3 flex flex-col bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                      Function
                      <span className="text-xs font-light text-gray-400">Instruction Pointer</span>
                    </th>
                  ...
                  {errors[index]["what"][0]["stack"].map((issue, j) => (
                    <tr className={`${j % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                      ...
                      <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">
                        {issue.dir}
                        <div className="text-gray-500">⎣ {issue.file}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-no-wrap truncate max-w-xs text-sm leading-5 text-gray-500">
                        {issue.obj}
                      </td>
                      <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">{issue.line}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

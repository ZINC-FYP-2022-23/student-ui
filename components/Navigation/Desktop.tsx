...
function CourseList({ courses }) {
  ...
  return (
    <div className="mt-8">
      ...
      <h3
        className="text-xs leading-4 font-semibold text-gray-500 uppercase tracking-wider"
        id="courselisting-headline"
      >
        Recent Courses
      </h3>
        ...
        {courses.map(({ course, role }) => (
          ...
          >
            <Switch
              ...
              onChange={(visible) =>
                setVisibleCourses(
                  visible ? [...visibleCourses, course.id] : visibleCourses.filter((id) => id !== course.id),
                )
              }
            />
...
export function Navigation({ courses }) {
  ...
  return (
    <div className="hidden md:block w-64 p-6 bg-gray-100 overflow-y-auto">
      <nav>
        <h2 className="text-xs font-semibold text-gray-600 uppercase tracking-wider">Overview</h2>
  ...
  return (
    <div>
      <button
        ...
        type="button"
        className="group w-full rounded-md px-3.5 py-2 text-sm leading-5 font-medium text-white hover:bg-gray-50 hover:text-gray-500 focus:outline-none focus:bg-gray-200 focus:border-blue-300 active:bg-gray-50 active:text-gray-800 transition ease-in-out duration-150"
        id="options-menu"
        aria-haspopup="true"
        aria-expanded="true"
      >
        <div className="flex w-full justify-between items-center">
          <div className="flex items-center justify-between space-x-3"></div>
        </div>
      </button>
    </div>
  );

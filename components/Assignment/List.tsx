    ...
    <Link href="/assignments/[assignmentId]" as={`/assignments/${id}`}>
      ...
      >
        ...
        <p className="text-sm text-gray-900">
          <span
            className={`mr-2 inline-flex items-center w-10 justify-center px-2 py-0.5 rounded-md text-xs font-semibold leading-5 ${
              assignmentColor[assignment.workloadType.name]
            }`}
          >
        ...
        <p className="text-sm text-gray-500 flex justify-start">
          ...
          <span>
            Due on {dueDate.toLocaleDateString("en-HK")} at {dueDate.toLocaleTimeString()}
          </span>
        </p>
        <p className="text-sm text-gray-500 flex justify-start">
          <span className="w-8">
...
export function Assignments() {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      {!loading &&
        data.user.assignedTasks.map(({ assignment_config: config }) => (
